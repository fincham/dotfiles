...
Matt Peterson <ricochet1k@gmail.com>
Ported to Python 3 by Michael Fincham <michael@hotplate.co.nz>
...
def _eval(expr, l):
    return eval(expr, _titen_globals, l)
...
def _do_list(list):
    if len(list) == 1 and type(list[0]) == types.FunctionType:
        return list[0]
...
def _def(func_name, params, f):
    if params == None:
        ...
        params = list(map(strip, params.split(",")))
...
_def.re = re.compile(
    r"\{\s*def\s+([a-zA-Z_][a-zA-Z0-9_]*)(?:\(([a-zA-Z_][a-zA-Z0-9_]*(?:\s*,\s*[a-zA-Z_][a-zA-Z0-9_]*)*)?\))?\}"
)
_def.needs_end = True
