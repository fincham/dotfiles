...
batteries = [entry for entry in os.listdir('/sys/class/power_supply') if entry.startswith('BAT')]

# look for wireless interfaces to configure i3status etc
wifi_interfaces = [interface.split('/')[-1] for interface in os.listdir('/sys/class/net') if 'phy80211' in os.listdir('/sys/class/net/%s' % interface)]
